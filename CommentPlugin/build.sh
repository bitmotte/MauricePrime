...
echo ""
echo "building. . ."
dotnet build "CommentMod.sln"

userpath=$(< "./resources/buildsettings" )
if [ -d "${userpath}plugins/CommentMod/" ]; then
    echo "destroying existing files"
    rm -r -f "${userpath}plugins/CommentMod/"
fi

echo "copying build to ultrakill folders"
mkdir "${userpath}plugins/CommentMod/"
cp "./bin/Debug/netstandard2.1/bitmotte.CommentMod.dll" "${userpath}plugins/CommentMod/"
echo "done! ! !"
